...
fn test_only_witness_utxo_with_finish() {
    ...
    if !has_utxos {
        ...
        let address = wallet.next_unused_address(bdk_wallet::KeychainKind::External).address;
    ...
    if let (Ok(Some(ext_policy)), Ok(Some(int_policy))) = (ext_policy, int_policy) {
        ...
        match (tx_without_flag, tx_with_flag) {
            (Ok(psbt_without), Ok(psbt_with)) => {
                ...
                println!("PSBT size without only_witness_utxo: {} bytes", size_without);
...
fn test_add_foreign_utxo_missing_witness_data() {
    ...
    let outpoint = OutPoint {
        txid: Arc::new(crate::bitcoin::Txid::from_string(
    ...
    if let Err(e) = finish_result {
        ...
        assert!(
            error_msg.contains("missing") || error_msg.contains("Utxo") || error_msg.contains("utxo"),
...
fn test_add_foreign_utxo_with_witness_utxo_succeeds() {
    ...
    let witness_utxo = TxOut {
        ...
        script_pubkey: Arc::new(Script::new(Vec::from_hex("0014d85c2b71d0060b09c9886aeb815e50991dda124d").unwrap())),
    };
    ...
    let result = tx_builder.add_foreign_utxo(outpoint, psbt_input, 68);
    assert!(result.is_ok(), "Failed to add foreign UTXO: {:?}", result.err());
...
fn test_add_multiple_foreign_utxos_and_finish() {
    ...
    let witness_utxo2 = TxOut {
        ...
        script_pubkey: Arc::new(Script::new(Vec::from_hex("0014a85c2b71d0060b09c9886aeb815e50991dda125e").unwrap())),
    };
    ...
    let tx_builder = tx_builder.add_foreign_utxo(outpoint1, psbt_input1, 68)
        .expect("Failed to add first foreign UTXO");

    let tx_builder = tx_builder.add_foreign_utxo(outpoint2, psbt_input2, 68)
    ...
    if let Err(e) = finish_result {
        let error_msg = format!("{:?}", e);
        println!("Transaction with multiple foreign UTXOs failed with: {}", error_msg);
        assert!(
...
fn test_combined_only_witness_utxo_and_foreign_utxo_with_finish() {
    ...
    let tx_builder = tx_builder.add_foreign_utxo(outpoint, psbt_input, 68)
    ...
    if let Err(e) = finish_result {
        ...
        assert!(
            !error_msg.contains("MissingUtxo") && !error_msg.contains("witness") || error_msg.contains("Insufficient"),
